...
class extension(Script):
    ...
    def getSettingDataString(self):
        return """{
            ...
            "settings":
            {
                ...
                "tool_change":
                {
                    "label": "Modify Tool-Change Behavior",
                    "description": "When enabled, the tool-change functions will be modified to be suitable for Diabase machines.",
                    "type": "bool",
                    "default_value": false
                },
                "preheat":
    ...
    def execute(self, data):
        new_data, new_layer, removedT = [], '', False
        if self.getSettingValueByKey("opening_lines"):
            for layer_number, layer in enumerate(
                ...
                for line in lines_in_layer:  # Loop to move through every line in each layer
                    ...
                    if removedT is False and len(line) is 2 and "T" in line:
                        line = ''  # Replace the T command with an empty string
                        removedT = True  # This indicates that we no longer need to search for the initial T command
        ...
        if self.getSettingValueByKey("tool_change"):
            pass
